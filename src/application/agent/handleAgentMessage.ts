...
import { buildProjectAgentTools } from "../../domain/agent/toolBuilder.js";
import { getProjectAgentRunner } from "../../domain/agent/runner.js";
import { z } from "zod";
import {
  loadProjectAgent,
  type AgentScript,
...
} from "../reporting/transactionalEmails.js";
import {
...
function updateMeetingProfileFromMessage(profile: MeetingProfile, message: string, lead: LeadProfile): MeetingProfile {
  ...
  if (timeMatch) {
    ...
    // Catch "a las 3", "las 3 pm", "3 de la tarde", "3pm", etc.
    const looseTimeMatch = text.match(
      /(?:a\s+las?\s+|las?\s+)([01]?\d|2[0-3])\s*(am|pm|de\s+la\s+(?:ma[nñ]ana|tarde|noche))?/i
    ) ?? text.match(/\b([01]?\d|2[0-3])\s*(am|pm|de\s+la\s+(?:ma[nñ]ana|tarde|noche))\b/i);
    if (looseTimeMatch?.[1]) {
      const hour = looseTimeMatch[1];
      const suffix = looseTimeMatch[2] ? ` ${looseTimeMatch[2].trim()}` : "";
      next.meetingTime = `${hour}:00${suffix}`;
    }
  }
...
function assistantRecentlyAskedMeetingTime(state: ConversationState) {
  return recentAssistantMessages(state, 2).some((item) =>
    /(?:hora\s+exacta|a\s+qu[eé]\s+hora|qu[eé]\s+hora|hora\s+de\s+la\s+reuni[oó]n|qu[eé]\s+horario|horario\s+prefer)/i.test(item.text)
  );
}

function messageLooksLikeTimeAnswer(message: string) {
  return (
    /(?:a\s+las?\s+|las?\s+)([01]?\d|2[0-3])/i.test(message) ||
    /\b([01]?\d|2[0-3])\s*(am|pm|de\s+la\s+(?:ma[nñ]ana|tarde|noche))\b/i.test(message) ||
...
function messageAdvancesActiveProcess(
  ...
  // Secondary guard: if the assistant just asked for the time and the reply looks like a time, treat it as advancing.
  if (assistantRecentlyAskedMeetingTime(state) && messageLooksLikeTimeAnswer(message)) {
    return true;
  }
